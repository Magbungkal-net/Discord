DiscordMarketplaceCommand:
    ...
    - definemap options:
       ...
       2:
         type: string
         name: server
         description: what server? (skyblock or survival)
         required: true
       3:
         type: integer
         name: price
...
DiscordMarketplaceCommandEvents:
    ...
    events:
      ...
      - define marketplace_server <context.options.get[server]>
      ...
      - if <[marketplace_selling_buying].starts_with[b]>:
         - definemap message_map:
              ...
              color: orange
              description: **Server:** **`<[marketplace_server].to_uppercase>`**<n>**Item:** **`<[marketplace_item].to_uppercase>`**<n>**Condition:** **`<[condition].to_uppercase>`**<n>**Price:** **`₱<[marketplace_price]>`**<n>**```<[marketplace_description]>```**<n>**Date:** <[time]><n><n>Do direct message <[user].mention> if you have the said item, or interested selling the said item.
      ...
      - if <[marketplace_selling_buying].starts_with[s]>:
         - definemap message_map:
              ...
              title: "<[user].name> is <[marketplace_selling_buying].to_uppercase>"
              color: orange
              description: **Server:** **`<[marketplace_server].to_uppercase>`**<n>**Item:** **`<[marketplace_item].to_uppercase>`**<n>**Condition**: **`<[condition].to_uppercase>`**<n>**Price:** **`₱<[marketplace_price]>`**<n>**```<[marketplace_description]>```**<n>**Date:** <[time]><n><n>Do direct message <[user].mention> if you are interested/have a question on this item.
              image: <[marketplace_image]>
         - define embed <discord_embed.with_map[<[message_map]>]>
         - ~discordmessage id:magbungkal channel:1261743849731260476 <[embed]>
