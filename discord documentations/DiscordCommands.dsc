discord_commands_events:
    ...
    events:
        ...
        - if <[first_word].is_in[<[commands_list]>]>:
            ...
            - if <[has_button]>:
                - ~discordmessage id:magbungkal reply:<[message]> <[embed_message].with[color].as[#7C4751]> rows:<[button]>
            - else:
                - ~discordmessage id:magbungkal reply:<[message]> <[embed_message].with[color].as[#7C4751]>